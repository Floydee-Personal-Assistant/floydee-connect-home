...
export default function App() {
  ...
  const [mobileDemoFocus, setMobileDemoFocus] = useState(false);
  const [interestReady, setInterestReady] = useState(false);
  const iframeRef = useRef<HTMLIFrameElement>(null);
  const demoRef = useRef<HTMLElement>(null);
  const mobileDemoFocusedRef = useRef(false);
  ...
  useEffect(() => {
    if (!window.matchMedia("(max-width: 760px) and (pointer: coarse)").matches) return undefined;
    let lastScrollY = window.scrollY;
    let sawDownwardScroll = false;
    const focusDemo = () => {
      if (mobileDemoFocusedRef.current) return;
      mobileDemoFocusedRef.current = true;
      setMobileDemoFocus(true);
      window.requestAnimationFrame(() => demoRef.current?.scrollIntoView({ block: "start", behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth" }));
    };
    const onScroll = () => {
      const currentScrollY = window.scrollY;
      if (currentScrollY > lastScrollY + 8) sawDownwardScroll = true;
      lastScrollY = currentScrollY;
      const demo = demoRef.current;
      if (!sawDownwardScroll || !demo || mobileDemoFocusedRef.current) return;
      const bounds = demo.getBoundingClientRect();
      if (bounds.top < window.innerHeight * 0.82 && bounds.bottom > 0) focusDemo();
    };
  ...
  useEffect(() => {
    if (demoReady) iframeRef.current?.contentWindow?.postMessage({ type: "floydee-prototype-presentation", mobileFocus: mobileDemoFocus }, window.location.origin);
  }, [demoReady, mobileDemoFocus]);
  ...
  return <main className="showcase" id="top" data-page-ready={pageReady} data-mobile-demo-focus={mobileDemoFocus || undefined}>
    ...
    <section className="showcase-layout" aria-labelledby="showcase-title">
      ...
      <aside ref={demoRef} className="showcase-demo" aria-label="Interactive Floydee Connect prototype" data-tour-stage={activeTourStage ?? "manual"} data-demo-ready={demoReady} data-demo-activated={demoActivated} data-mobile-focus={mobileDemoFocus || undefined}><div className="showcase-phone">{pageReady ? <iframe ref={iframeRef} src={embeddedPrototypeUrl} title="Interactive Floydee Connect prototype" onLoad={() => setDemoReady(true)} /> : null}{!demoActivated ? <p className="showcase-demo-callout"><span>Try it</span><strong>Click the phone to explore.</strong><ArrowDownRight aria-hidden="true" size={24} /></p> : null}{!demoActivated ? <button className="showcase-demo-activator" type="button" onClick={activateDemo} aria-label="Start exploring the Floydee Connect prototype" /> : null}</div><p className="showcase-demo-status" aria-live="polite"><span>{demoReady ? "Live prototype" : "Preparing prototype"}</span>{mobileDemoFocus ? "Tap the phone to explore." : activeTourStage ? tourCopy[activeTourStage] : "Hover or tap the phone to take control."}</p></aside>
    </section>
