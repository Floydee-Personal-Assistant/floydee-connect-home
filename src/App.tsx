...
export default function App() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => entries.forEach((entry) => { if (entry.isIntersecting) entry.target.setAttribute("data-revealed", "true"); }), { threshold: 0.3 });
    document.querySelectorAll<HTMLElement>(".showcase-chapter, .showcase-pilot").forEach((element) => observer.observe(element));
  ...
  return <main className="showcase" id="top" data-page-ready={pageReady}>
    ...
    <section className="showcase-layout" aria-labelledby="showcase-title">
      ...
      <aside className="showcase-demo" aria-label="Interactive Floydee Connect prototype" data-tour-stage={activeTourStage ?? "manual"} data-demo-ready={demoReady} data-demo-activated={demoActivated}><div className="showcase-phone">{pageReady ? <iframe ref={iframeRef} src={prototypeUrl} title="Interactive Floydee Connect prototype" onLoad={() => setDemoReady(true)} /> : null}{!demoActivated ? <button className="showcase-demo-callout" type="button" onClick={activateDemo} onFocus={activateDemo} aria-label="Open interactive prototype"><span>Try it</span><strong>Click the prototype to explore your plan.</strong><ArrowDownRight aria-hidden="true" size={44} /></button> : null}</div><p className="showcase-demo-status" aria-live="polite"><span>{demoReady ? "Live prototype" : "Preparing prototype"}</span>{activeTourStage ? tourCopy[activeTourStage] : "Hover or tap the phone to take control."}</p></aside>
    </section>
    <section className="showcase-chapter showcase-chapter-context" id="context" aria-labelledby="context-title"><div className="chapter-copy"><p>01 / Context</p><h2 id="context-title">Capture what matters.</h2><span>From deliberate device capture and approved sources, context arrives with permission and proof.</span></div><div className="chapter-visual source-stream" aria-label="Permitted context sources"><i><Smartphone aria-hidden="true" size={18} />Device capture</i><i><CalendarDays aria-hidden="true" size={18} />Calendar</i><i><Mic aria-hidden="true" size={18} />Meetings</i><i><Mail aria-hidden="true" size={18} />Mail</i><i><MessageCircle aria-hidden="true" size={18} />Work tools</i></div></section>
    <section className="showcase-chapter showcase-chapter-intelligence" aria-labelledby="intelligence-title"><div className="chapter-copy"><p>02 / Intelligence</p><h2 id="intelligence-title">Understand the signal.</h2><span>Topics, commitments, goals, and evidence become clear enough to review and correct.</span></div><div className="chapter-visual intelligence-packet" aria-label="Reviewable intelligence packet"><span>Topic</span><b>Release readiness</b><small>Goal · Ship Capture V1</small><em>Capacity checked</em></div></section>
    <section className="showcase-chapter showcase-chapter-plan" aria-labelledby="plan-title"><div className="chapter-copy"><p>03 / Plan</p><h2 id="plan-title">Protect the promise.</h2><span>See realistic capacity, spot a risk early, and choose the least-disruptive next step.</span></div><div className="chapter-visual plan-signal" aria-label="A planned commitment"><Target aria-hidden="true" size={22} /><span><b>Next commitment</b><small>Review delivery options</small></span><i>Ready to review</i></div></section>
    <section className="showcase-pilot" aria-labelledby="pilot-title"><div><p className="showcase-kicker">Early pilot</p><h2 id="pilot-title">Build the planning layer with us.</h2><p>For a small group of teams that want to make commitments, capacity, and delivery more visible.</p><small>Share your details to open an email draft. Nothing is sent from this site.</small></div><form className="pilot-form" onSubmit={openInterestDraft}><label>Name<input name="name" autoComplete="name" required /></label><label>Work email<input name="email" type="email" autoComplete="email" required /></label><label>Company<input name="company" autoComplete="organization" required /></label><label>Team size <select name="team" defaultValue=""><option value="" disabled>Select one</option><option>1–10</option><option>11–50</option><option>51–250</option><option>251+</option></select></label><button type="submit" data-loading={interestReady || undefined}>Show interest <ArrowUpRight aria-hidden="true" size={17} /></button>{interestReady ? <p role="status">Your email draft is ready to review.</p> : null}</form></section>
    <footer className="showcase-footer"><p>© 2026 Floydee Innovations Private Limited</p><p className="showcase-trust"><ShieldCheck aria-hidden="true" size={16} />Your data stays protected. You stay in control.</p><a href="mailto:admin@floydee.com">admin@floydee.com</a></footer>
  </main>;
}
