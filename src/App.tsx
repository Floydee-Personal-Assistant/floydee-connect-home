...
export default function App() {
  ...
  return <main className="showcase" id="top" data-page-ready={pageReady}>
    ...
    <section className="showcase-layout" aria-labelledby="showcase-title">
      ...
      <aside className="showcase-demo" aria-label="Interactive Floydee Connect prototype" data-tour-stage={activeTourStage ?? "manual"} data-demo-ready={demoReady} data-demo-activated={demoActivated}>{!demoActivated ? <p className="showcase-demo-callout"><span>Try it</span><strong>Click the phone to explore.</strong><ArrowDownRight aria-hidden="true" size={28} /></p> : null}<div className="showcase-phone">{pageReady ? <iframe ref={iframeRef} src={embeddedPrototypeUrl} title="Interactive Floydee Connect prototype" onLoad={() => setDemoReady(true)} /> : null}{!demoActivated ? <button className="showcase-demo-activator" type="button" onClick={activateDemo} aria-label="Start exploring the Floydee Connect prototype" /> : null}</div><p className="showcase-demo-status" aria-live="polite"><span>{demoReady ? "Live prototype" : "Preparing prototype"}</span>{activeTourStage ? tourCopy[activeTourStage] : "Hover or tap the phone to take control."}</p></aside>
    </section>
